//! 采集服务 - 使用 ai-cli-session-db::Collector 统一业务逻辑
//!
//! 此模块是对 ai-cli-session-db::Collector 的薄包装，
...
// Re-export CollectResult from ai-cli-session-db
pub use ai_cli_session_db::CollectResult;

/// 采集服务（薄包装）
///
/// 内部调用 ai-cli-session-db::Collector，确保业务逻辑统一。
#[derive(Clone)]
...
impl Collector {
    ...
    /// 执行全量采集
    ///
    /// 使用 ai-cli-session-db::Collector 统一的采集逻辑。
